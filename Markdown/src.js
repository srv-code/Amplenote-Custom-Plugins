...
a = {
  constants: {
    settings: {
      ...
      ONLY_ALLOW_EXPERIMENTS_ON_TEST_NOTES:
        '🧪 Only allow experimental features on notes tagged with `*/test/*`? (y/n | default: y)',
    },
    messages: {
  ...
  _getSettingValue(app, settingName, type = 'string', defaultValue = null) {
    ...
    if (type === 'boolean')
      return val?.length > 0 ? val.trim().toLowerCase() === 'y' : defaultValue || false;
    if (type === 'string') return val || defaultValue;
  },

  async _hasEnabledExperiments(app) {
    const experimentsEnabled = this._getSettingValue(
      app,
      this.constants.settings.ENABLED_EXPERIMENTS,
      'boolean',
    );
    if (!experimentsEnabled) return false;

    const onlyOnTestNotes = this._getSettingValue(
      app,
      this.constants.settings.ONLY_ALLOW_EXPERIMENTS_ON_TEST_NOTES,
      'boolean',
      true,
    );
    ...
    const hasTestTag = note?.tags.some(tag =>
      tag.split('/').includes(this.constants._.TEST_TAG_NAME),
    );
    return hasTestTag;
  },
    ...
    View: {
      async run(app, noteUUID) {
        try {
          ...
          await app.alert(content, {
            preface: this.constants.messages.VIEWING_NOTE_MARKDOWN.replace('$1', content.length),
          });
        } catch (error) {
      ...
      async run(app, text) {
        try {
          await app.alert(app.context.selectionContent, {
            preface: this.constants.messages.VIEWING_SELECTION_MARKDOWN.replace(
              '$1',
              app.context.selectionContent.length,
            ),
          });
        } catch (error) {
          this._handleError(app, error);
