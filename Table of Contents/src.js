...
a = {
  constants: {
    ...
    messages: {
      ERROR_INVALID_TITLE_SETTING_VALUE:
        'Invalid value provided for "$1". Please remove the following characters: $2',
  ...
  validateSettings(app, settings) {
    ...
    const {
      ...
      messages: { ERROR_INVALID_TITLE_SETTING_VALUE, ERROR_INVALID_BOOLEAN_SETTING },
    } = this.constants;
      ...
      if (value?.length > 0) {
        if (setting.type === 'string') {
          if (
            ...
            errors.push(
              ERROR_INVALID_TITLE_SETTING_VALUE.replace('$1', name).replace(
                '$2',
                INVALID_SECTION_TITLE_CHARS.join(' '),
              ),
            );
          }
  ...
  _getNextIndex(index, level, unordered) {
    ...
    const { BULLET_H1_CHAR, BULLET_H2_CHAR, BULLET_H3_CHAR } = this.constants._;
  ...
  _generateTOCFromSections(
    ...
    for (const { heading, index } of sections) {
      ...
      if (!!heading && !!heading.text.trim() && (!isTOC || (isTOC && !!section))) {
        const headingLevel = considerHeaderBreaks
  ...
  _getSettingValue(app, settingKey) {
    ...
    if (type === 'boolean')
      return val?.length > 0 ? val.trim().toLowerCase() === 'y' : defaultValue === 'y' || false;
  ...
  __diag__checkSettingsParsing(app) {
    ...
    console.log('Parsed', {
      ...
      WARN_ON_OFFENDING_SECTION_TITLES: this._getSettingValue(
        app,
        'WARN_ON_OFFENDING_SECTION_TITLES',
      ),
  ...
  async _exec(app, noteUUID, insertOnly = false, showAlertOnSuccess = false) {
    try {
      ...
      const {
        _: { INVALID_MODES, UUID_REGEX, INVALID_SECTION_TITLE_CHARS },
        messages: {
      ...
      if (
        sections.length === 0 ||
        sections.every(sec => sec.heading === null || sec.heading.text.trim().length === 0)
      ) {
      ...
      if (this._getSettingValue(app, 'WARN_ON_DUPLICATE_TITLES')) {
        const dupSections = sections.filter(
          sec => sec.heading != null && sec.heading.text.length > 0 && sec.index != null,
        );
        ...
        if (dupSections.length > 0) {
          ...
          const message = ERROR_DUPLICATE_SECTION_BODY.replace('$1', dupSections.length)
            .replace('$2', headers)
            .trim();
          ...
          console.log(
            'Alert response for duplicate sections:',
            response === -1 ? 'ABORT' : response,
          );
      ...
      if (this._getSettingValue(app, 'WARN_ON_OFFENDING_SECTION_TITLES')) {
        const offendingSections = sections.filter(
          sec =>
            sec.heading != null &&
        ...
        if (offendingSections.length > 0) {
          ...
          for (const index in offendingSections) {
            ...
            const chars = INVALID_SECTION_TITLE_CHARS.filter(ch =>
              section.heading.text.includes(ch),
            ).join(' ');
          ...
          const message = ERROR_OFFENDING_SECTION_BODY.replace('$1', offendingSections.length)
            .replace('$2', headers)
            .trim();
          ...
          console.log(
            'Alert response for offending sections:',
            response === -1 ? 'ABORT' : response,
          );
        ...
        if (section) {
          ...
          if (preventIdenticalRewrite) {
            ...
            if (extractedTOCLines === null) {
              console.error(
                'Unexpected Error: TOC section not found in the note content, even though it was detected in the sections.',
                {
                  noteContent,
                  extractedTOCLines,
                  tocLines,
          ...
          if (requireUpdate) {
            ...
            result = await app.replaceNoteContent({ uuid: noteUUID }, tocLines.join(''), {
              section,
            });
          ...
          result = await app.insertNoteContent(
            { uuid: noteUUID },
            this._getTOCSection(title, tocLines.join('')),
          );
          if (showAlertOnSuccess) {
  ...
  noteOption: {
    ...
    async run(app, noteUUID) {
      console.groupCollapsed('TOC plugin | running from `noteOption`', {
        noteUUID,
      });
  ...
  appOption: {
    ...
    async run(app) {
      ...
      console.groupCollapsed('TOC plugin | running from `appOption`', {
        noteUUID,
        url: app.context.url,
      });
  ...
  insertText: {
    ...
    async run(app) {
      ...
      console.groupCollapsed('TOC plugin | running from `insertText`', {
        noteUUID,
      });
